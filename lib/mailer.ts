// lib/mailer.ts — Nodemailer com Gmail (substitui Resend)
import nodemailer from 'nodemailer';

function getTransporter() {
  // Instanciado dentro da função — nunca executa em build time
  ...
  return getTransporter().sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#0f0f0f;color:#f5f5f5;border-radius:16px">
        ...
        <h1 style="font-size:22px;font-weight:900;margin:0 0 8px">Bem-vindo(a) ao Finexa, ${nome}! 👋</h1>
        <p style="color:#888;margin:0 0 24px;line-height:1.6">Sua conta foi criada com sucesso. Clique abaixo para acessar o app.</p>
        <a href="${confirmUrl}" style="display:inline-block;background:#5330ff;color:#fff;padding:14px 28px;border-radius:12px;text-decoration:none;font-weight:900;font-size:15px">Acessar o Finexa →</a>
        <p style="color:#555;font-size:12px;margin-top:24px">Seu dinheiro, com clareza.</p>
      </div>
    `,
  });
}
...
export async function enviarEmailConvite(to: string, nomeFamilia: string, nomeQuemConvidou: string, conviteUrl: string) {
  return getTransporter().sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#0f0f0f;color:#f5f5f5;border-radius:16px">
        <div style="width:48px;height:48px;background:#5330ff;border-radius:12px;font-size:24px;font-weight:900;color:#fff;line-height:48px;text-align:center;margin-bottom:24px">F</div>
        <h1 style="font-size:22px;font-weight:900;margin:0 0 8px">Você foi convidado(a)! 💜</h1>
